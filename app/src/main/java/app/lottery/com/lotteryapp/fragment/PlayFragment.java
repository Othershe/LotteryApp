...
import app.lottery.com.lotteryapp.plays.BlueBallAdapter;
import app.lottery.com.lotteryapp.plays.Calculator;
import app.lottery.com.lotteryapp.plays.RandomMadeBall;
import app.lottery.com.lotteryapp.plays.RedBallAdapter;
import app.lottery.com.lotteryapp.plays.ShakeListener;
...
public class PlayFragment extends BaseFragment implements View.OnClickListener {
    ...
    @BindView(R.id.gridview_hongqiu)
    GridView gridViRedBall;

    @BindView(R.id.gridview_lanqiu)
    GridView gridViBlueBall;

    @BindView(R.id.txt_showhongqiu)
    TextView txtShowRedBall;

    @BindView(R.id.txt_showlanqiu)
    TextView txtShowBlueBall;

    @BindView(R.id.txt_result)
    TextView txtResult;

    @BindView(R.id.btn_random_select)
    TextView btnRandomSelect;

    @BindView(R.id.btn_clear_number)
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                for (int i = 0; i < gridViRedBall.getCount(); i++) {
                    ...
                    if (adpRedBall.hisSelected.get(i)) {
                        ...
                        vHolder_red.chkRed.setTextColor(ContextCompat.getColor(mActivity, android.R.color.white));
                    } else {
                        vHolder_red.chkRed.setTextColor(ContextCompat.getColor(mActivity, android.R.color.black));
                    }
    ...
    private void init() {
        // 获得振动器服务
        mVibrator = (Vibrator) mActivity.getApplication().getSystemService(mActivity.VIBRATOR_SERVICE);
        // 实例化加速度传感器检测类
        mShakeListener = new ShakeListener(mActivity);

        btnRandomSelect.setOnClickListener(this);
        btnClearNumber.setOnClickListener(this);

        adpRedBall = new RedBallAdapter(mActivity, arrRandomRed);
        adpBlueBall = new BlueBallAdapter(mActivity, arrRandomBlue);
